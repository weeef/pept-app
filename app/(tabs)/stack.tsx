    ...
    <View className="flex-1 bg-[#F2F2F7]">
      ...
      >
        <View className="px-1 mt-2 mb-8">
          <Text className="text-3xl font-bold tracking-tight mb-1 text-black">Your Stack</Text>
          <Text className="text-gray-500 font-medium">Manage compounds & shelf life.</Text>
        </View>
        ...
        <View className="gap-4">
          {stack.map((item) => {
            ...
            return (
              <View key={item.id} className="bg-white border border-gray-100 p-5 rounded-[28px] shadow-sm mb-2">
                <View className="flex-row items-start justify-between mb-4">
                  <View className="flex-row items-center gap-4 flex-1">
                    <View className={`w-14 h-14 rounded-2xl items-center justify-center ${item.type === 'peptide' ? 'bg-blue-500' : 'bg-[#30B0C7]'}`}>
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="text-sm font-medium text-gray-500">{item.dose} • {item.frequency}</Text>
                    </View>
                  </View>
                ...
                {item.type === 'peptide' && daysLeft !== null && (
                  ...
                  >
                    ...
                    <Text className={`text-[13px] font-semibold flex-1 leading-relaxed ${isExpired ? 'text-red-700' : isExpiringSoon ? 'text-orange-700' : 'text-blue-700'}`}>
                      {isExpired 
