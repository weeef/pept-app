  ...
  return (
    ...
    >
      <View className="px-1 mt-2 mb-6">
        <Text className="text-3xl font-bold tracking-tight mb-1 text-black">Liquid Math</Text>
        <Text className="text-gray-500 font-medium">Safe, visual reconstitution.</Text>
      </View>
      ...
      <View className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-100 mb-6">
        ...
        <View className="mb-4">
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-3">
              <Text className="text-sm font-bold text-gray-700">Peptide Vial Size</Text>
          ...
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-3">
              <Text className="text-sm font-bold text-gray-700">BAC Water Added</Text>
              <Text className="text-[#14B8A6] text-lg font-bold">{bacMl} mL</Text>
            </View>
            <Slider
              minimumValue={0.5}
              maximumValue={5}
          ...
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-3">
              <Text className="text-sm font-bold text-gray-700">Desired Target Dose</Text>
              <Text className="text-[#6366F1] text-lg font-bold">{doseMcg} mcg</Text>
            </View>
            <Slider
              minimumValue={50}
              maximumValue={2500}
        ...
        {/* Visual Result */}
        <View className="bg-gray-50 border border-gray-200 rounded-3xl p-6 items-center relative overflow-hidden">
          <Text className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Draw to line on U-100 Syringe</Text>
          <View className="flex-row items-baseline justify-center gap-2 mb-4">
            <Text className={`${unitsFontSize} font-black text-black tracking-tighter`}>{displayValue}</Text>
            <Text className="text-xl font-bold text-gray-400">{displayLabel}</Text>
          </View>
          
          <View className="mt-8 flex-row justify-between items-end border-b-2 border-gray-300 pb-2 px-2 w-full z-10">
            {[0, 5, 10, 15, 20, 25, 30].map(tick => (
               <View key={tick} className="items-center">
                 <Text className="text-[10px] text-gray-400 font-bold mb-1">{tick}</Text>
                 <View className={`w-0.5 ${tick % 10 === 0 ? 'h-5 bg-gray-500' : 'h-3 bg-gray-300'}`} />
               </View>
            ))}
          </View>
          <View 
